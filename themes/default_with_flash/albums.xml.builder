# Serves the album images, title, and descriptions for the front page slides.
...
xml.cu3er do
  ...
  xml.slides do
    for album in @albums
      xml.slide do
        xml.url album.image.url
        xml.link album_path(album), :target => "_self"
        xml.description do
          xml.link album_path(album), :target => "_self"
          xml.heading album.title
          xml.paragraph album.description
        end
      end
    end
  end
